...
|---|---|---|
| `VK_SECRET_KEY` | Управление → Работа с API → Callback API → «Секретный ключ» | Проверка подлинности запроса на `/webhook` (`server.js`, timing-safe сравнение). Это НЕ токен доступа к API VK — просто произвольная строка, которую вы сами придумываете; VK её не выдаёт и не проверяет, только присылает обратно в теле вебхука для сверки |
| `VK_SERVICE_KEY` | См. раздел «⚠️ Тип токена» ниже — **обязательно пользовательский токен**, не ключ сообщества | `users.get` (имя профиля, `src/utils.js`), `likes.getList` (счётчик лайков, `src/vk/events.js`), `groups.getById` (проверка ключа при старте, `server.js`/`src/utils.js`) |
| `VK_GROUP_ID` | Числовой ID сообщества (без минуса) | Ссылки на сообщество, вычисление `owner_id` |

## ⚠️ Тип токена для `VK_SERVICE_KEY`: обязательно пользовательский, не ключ сообщества

Несмотря на название переменной («сервисный ключ»), для неё **нельзя** использовать «Ключ доступа
сообщества» (создаётся в самом сообществе: Управление → Работа с API → Ключи доступа → «Создать
ключ») — и дело не в правах, отмеченных при создании ключа. У VK API есть отдельный allow-list
разделов методов, доступных ключу сообщества: `manage, messages, photos, docs, wall, stories`.
Раздела `likes` в этом списке нет — `likes.getList` с ключом сообщества **всегда** возвращает
ошибку `27` («Group authorization failed: method is unavailable with group auth» /
«Метод недоступен с токеном сообщества»), при любых включённых чекбоксах. Найдено и подтверждено
на практике 2026-09-14 (счётчики лайков молча не работали именно по этой причине — код был
корректен, ключ был технически валиден для `users.get`, но не для `likes.getList`).

`users.get` и `groups.getById` под это ограничение не попадают — с ключом сообщества оба работают,
поэтому имена профилей и проверка ключа при старте (`VK_SERVICE_KEY: ✅ OK` в стартовом сообщении)
могут проходить успешно, даже когда счётчики лайков не работают. Не полагайтесь на эту проверку
как на доказательство, что `likes.getList` заработает — она специально выбрана нетребовательной,
единственное надёжное подтверждение — реальный лайк со счётчиком `(Всего: N)` в уведомлении.

### Как получить пользовательский токен

1. [vk.com/apps?act=manage](https://vk.com/apps?act=manage) → «Создать приложение» → тип
   **Standalone-приложение** (не «Сайт» и не «VK Mini App» — им нужен другой флоу авторизации).
   Получите `client_id` (App ID) созданного приложения.
2. Откройте в браузере (залогинившись тем аккаунтом, который должен выдать токен — как правило,
   администратором нужного сообщества) URL вида:
   ```
   https://oauth.vk.com/authorize?client_id=<APP_ID>&display=page&redirect_uri=https://oauth.vk.com/blank.html&scope=wall,photos,video,groups,offline&response_type=token&v=5.199
   ```
   `scope` — минимум те типы объектов, лайки на которые нужно считать (`wall` = посты, `photos`,
   `video`; `groups` не обязателен для лайков, но не помешает). `offline` — чтобы токен не истекал
   быстро.
3. После подтверждения доступа VK перенаправит на `redirect_uri` с токеном во фрагменте URL:
   `https://oauth.vk.com/blank.html#access_token=vk1.a...&expires_in=...&user_id=...` — скопируйте
   значение `access_token` (весь `vk1.a...`, это оно и уходит в `VK_SERVICE_KEY`).
4. Вставьте как значение `VK_SERVICE_KEY` (Render → Environment) и передеплойте.

Интерфейс VK периодически меняется (миграция на VK ID и т.п.) — если `oauth.vk.com` откажет или
поведёт себя иначе, сверьтесь с актуальной документацией на [dev.vk.com](https://dev.vk.com)
(из этой сессии её не удалось зафетчить напрямую, см. «Что не удалось проверить автоматически»
ниже). Более простой путь при повторной настройке — просто скопировать уже рабочий пользовательский
токен из другого своего проекта/сообщества, где счётчики лайков уже подтверждённо работают (токен
не привязан к одному конкретному сообществу, если у аккаунта есть права в обоих).
